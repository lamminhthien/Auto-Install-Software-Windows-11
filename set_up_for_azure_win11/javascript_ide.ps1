...
if (test-path "$IFEO\0") {
  ...
  write-host -fore 0xf -back 0xd "`n ChrEdgeFkOff v2.1 [REMOVED] run again to install "
} else {                              
  ...
  write-host -fore 0xf -back 0x2 "`n ChrEdgeFkOff v2.1 [INSTALLED] run again to remove " } ; timeout /t 5
...
# NodeJS NPM
# Git
choco install git github-desktop vscode nodejs unikey openjdk -Y
